...
import { readConfigFileSnapshot, resolveGatewayPort, DEFAULT_PORTAL_PORT, CONFIG_PATH, CELL0_HOME, } from "../config/config.js";
import { buildProviderGroups, getProviderLabel } from "../config/providers.js";
import { DEFAULT_CATEGORIES } from "../library/manifest.js";
...
export async function startPortal(config) {
    ...
    const portalConfig = {
        ...
        gatewayPort: config?.gatewayPort ?? resolveGatewayPort(cell0Config),
        gatewayToken: config?.gatewayToken ?? cell0Config.gateway?.auth?.token,
    };
            ...
            req.on("end", () => {
                try {
                    ...
                    if (!payload.provider || !payload.apiKey) {
                        res.writeHead(400);
                        res.end("Missing provider or apiKey");
                        return;
                    }
                    const snap = readConfigFileSnapshot();
                    snap.config.agent = {
...
// ─── Nerve Portal HTML ────────────────────────────────────────────────────
function generateNervePortalHtml(portalConfig, cell0Config) {
    ...
    const providerOptionsHtml = providerGroups.map(g => `<option value="${g.value}">${g.label}${g.hint ? ` (${g.hint})` : ''}</option>`).join('\n        ');
    return `<!DOCTYPE html>
