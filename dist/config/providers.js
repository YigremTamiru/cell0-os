...
// ─── Build Functions ──────────────────────────────────────────────────────
const optionByValue = new Map(AUTH_CHOICE_OPTIONS.map((opt) => [opt.value, opt]));
export function buildProviderGroups(includeSkip) {
    const groups = PROVIDER_GROUP_DEFS.map((group) => ({
        value: group.value,
        label: group.label,
        hint: group.hint,
...
export function requiresApiKey(choice) {
    const noKeyChoices = [
        "openai-codex",
        "chutes",
        "minimax-portal",
...
export function getApiKeyPrompt(choice) {
    const prompts = {
        "openai-api-key": {
