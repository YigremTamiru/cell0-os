...
import { makeResponse, makeError, makeEvent, } from "./protocol.js";
...
export class Gateway {
    ...
    async handleFrame(client, frame) {
        ...
        switch (type) {
            ...
            case "req":
                ...
                break;
            default:
                this.sendToClient(client, makeError(frame.id ?? "unknown", "INVALID_FRAME", `Unknown frame type: ${type}`));
        }
    }
    ...
    async handleRequest(client, frame) {
        // Idempotency check
    ...
    // ─── Method Router ──────────────────────────────────────────────────────
    async routeMethod(client, frame) {
        ...
        switch (frame.method) {
            ...
            case "session.get": {
                ...
                if (!session)
                    return makeError(frame.id, "NOT_FOUND", "Session not found");
                return makeResponse(frame.id, session);
            }
            case "session.delete": {
                const deleted = this.sessions.deleteSession(params.sessionId);
            ...
            case "chat.send": {
                ...
                if (this.python.isReady()) {
                    try {
                        const aiResp = await this.python.complete([{ role: "user", content: params.message }], params.model || "default");
                        const assistantMsg = this.sessions.addMessage(sessionId, {
                            role: "assistant",
                            content: aiResp.content || "[No response]",
                        });
    ...
    // ─── Python Proxy ───────────────────────────────────────────────────────
    async proxyToPython(frame, path, method, body) {
        if (!this.python.isReady()) {
    ...
    // ─── Event Broadcasting ─────────────────────────────────────────────────
    async findAvailablePort(startPort, host) {
...
const isDirectRun = process.argv[1]?.endsWith("gateway/index.js") ||
