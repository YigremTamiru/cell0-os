export class DomainRouter {
     ...
     */
    async route(message, defaultChannelDomain) {
    ...
    matchExplicitCommand(text) {
        if (text.startsWith('/social'))
            return { domain: 'social' };
        if (text.startsWith('/prod') || text.startsWith('/productivity'))
            return { domain: 'productivity' };
        if (text.startsWith('/finance'))
            return { domain: 'finance' };
        if (text.startsWith('/travel'))
            return { domain: 'travel' };
        if (text.startsWith('/creative') || text.startsWith('/creativity'))
            return { domain: 'creativity' };
        if (text.startsWith('/info') || text.startsWith('/information'))
            return { domain: 'information' };
        if (text.startsWith('/util') || text.startsWith('/utilities'))
