...
import { CONFIG_PATH, DEFAULT_GATEWAY_PORT, DEFAULT_PORTAL_PORT, DEFAULT_WORKSPACE, readConfigFileSnapshot, resolveGatewayPort, resolveUserPath, summarizeConfig, writeConfig, } from "../config/config.js";
import { buildProviderGroups, getApiKeyPrompt, getDefaultModelForProvider, requiresApiKey, } from "../config/providers.js";
// ─── Constants ────────────────────────────────────────────────────────────
const VERSION = "1.2.0";
// ─── Main Wizard ──────────────────────────────────────────────────────────
export async function runOnboardingWizard(opts) {
    ...
    if (snapshot.exists && !snapshot.valid) {
        p.note("Config file exists but has issues. Run `cell0 doctor` to repair.", "⚠️  Invalid Config");
    ...
    let workspaceDir;
    if (flow === "quickstart") {
        workspaceDir = resolveUserPath(baseConfig.agents?.defaults?.workspace ?? DEFAULT_WORKSPACE);
    }
    else {
        ...
        workspaceDir = resolveUserPath(wsInput.trim() || DEFAULT_WORKSPACE);
    }
    nextConfig.agents = {
        ...nextConfig.agents,
        defaults: {
    ...
    if (flow === "advanced") {
        const portInput = await p.text({
            ...
            initialValue: String(baseConfig.gateway?.port ?? DEFAULT_GATEWAY_PORT),
        });
        ...
        nextConfig.gateway = {
            ...nextConfig.gateway,
            port: parseInt(portInput, 10) || DEFAULT_GATEWAY_PORT,
            bind: bind,
        };
    }
    else {
    ...
    // 11. Daemon install
    const installDaemon = opts.installDaemon ??
        (flow === "quickstart"
            ? true
            : (await p.confirm({
    ...
    const tokenLine = authCfg?.mode === "token" && authCfg.token
        ? `Token:     ${authCfg.token}`
        : "";
    p.note([
...
async function requireRiskAcknowledgement(preAccepted) {
    if (preAccepted)
        return;
    p.note([
...
// ─── Channel Setup ────────────────────────────────────────────────────────
async function runChannelSetup(config) {
    ...
    while (true) {
        ...
        if (p.isCancel(channel) || channel === "done")
            break;
        config = await setupSingleChannel(channel, config);
        // After first add, relabel first option
        channelOptions[0].label = "Done — add more later with `cell0 configure`";
    }
    return config;
}
export async function setupSingleChannel(channel, config) {
    ...
    if (channel === "whatsapp") {
        ...
        if (!p.isCancel(tok) && tok) {
            ...
            await adapter.setup(tok.trim());
            channels.telegram = { enabled: true, dmPolicy: "open", token: tok.trim() };
        }
    }
        ...
        if (!p.isCancel(tok) && tok) {
            ...
            await adapter.setup(tok.trim());
            channels.discord = { enabled: true, dmPolicy: "open", token: tok.trim() };
        }
    }
        ...
        if (!p.isCancel(appTok) && !p.isCancel(botTok) && appTok && botTok) {
            ...
            await adapter.setup(appTok.trim(), botTok.trim());
            channels.slack = { enabled: true, dmPolicy: "open", appToken: appTok.trim(), botToken: botTok.trim() };
        }
    }
        ...
        if (!p.isCancel(phone) && phone) {
            ...
            p.note(`signal-cli -u ${num} register\nsignal-cli -u ${num} verify <code>`, "Run to register Signal");
        ...
        if (!p.isCancel(homeserver) && !p.isCancel(accessToken) && homeserver && accessToken) {
            channels.matrix = { enabled: true, dmPolicy: "open", homeserverUrl: homeserver.trim(), accessToken: accessToken.trim() };
        }
    }
        ...
        if (!p.isCancel(webhook) && webhook) {
            channels.googleChat = { enabled: true, webhookUrl: webhook.trim() };
        }
    }
        ...
        if (!p.isCancel(webhook) && webhook) {
            channels.msTeams = { enabled: true, webhookUrl: webhook.trim() };
        }
    }
        ...
        if (!p.isCancel(serverUrl) && !p.isCancel(serverPass) && serverUrl && serverPass) {
            channels.blueBubbles = { enabled: true, serverUrl: serverUrl.trim(), serverPass: serverPass.trim() };
...
// ─── Daemon Installer ─────────────────────────────────────────────────────
async function installDaemonService(config, token) {
    const s = p.spinner();
    ...
    try {
        ...
        if (process.platform === "darwin") {
            ...
            s.stop("Gateway service: manual start required (unsupported platform for auto-install).");
        }
    }
    catch (err) {
        s.stop("Gateway service install failed.");
        p.note(`Error: ${err instanceof Error ? err.message : String(err)}\n\nStart manually: cell0 gateway`, "⚠️  Service Install");
    }
}
async function installLaunchAgent(port, token) {
...
async function installSystemdService(port, token) {
