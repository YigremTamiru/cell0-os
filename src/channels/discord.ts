...
export class DiscordAdapter extends EventEmitter implements ChannelAdapter {
    ...
    private creds: DiscordCreds | null = null;
    ...
    private loadCreds(): DiscordCreds | null {
        if (!fs.existsSync(CREDS_PATH)) return null;
        try {
            return JSON.parse(fs.readFileSync(CREDS_PATH, "utf-8")) as DiscordCreds;
        } catch {
            return null;
        }
    }

    private async discordApi(method: string, endpoint: string, body?: unknown): Promise<any> {
    ...
    async connect(): Promise<void> {
        ...
        if (!this.creds?.botToken) {
            console.warn(
                "[Discord] No bot token. Create a bot at:\n" +
    ...
    private handleDispatch(type: string, data: any): void {
        switch (type) {
            ...
            case "MESSAGE_CREATE":
                if (data.author?.bot) return; // Ignore bots
    ...
    async setup(botToken: string): Promise<void> {
