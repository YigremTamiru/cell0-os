...
export function buildManifest(): LibraryManifest {
    return {
        version: "2026.2.19",
 ...
 */
export function matchCategory(keywords: string[]): Category | undefined {
    const lower = keywords.map(k => k.toLowerCase());
    let bestMatch: Category | undefined;
    let bestScore = 0;

    for (const cat of DEFAULT_CATEGORIES) {
        if (cat.dynamic && cat.routingKeywords.length === 0) continue;
        const score = cat.routingKeywords.filter(rk =>
            lower.some(k => k.includes(rk) || rk.includes(k))
        ).length;
        if (score > bestScore) {
            bestScore = score;
            bestMatch = cat;
