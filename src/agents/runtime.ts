...
export class AgentRuntime {
     ...
     */
    async executeTask(request: TaskRequest, options: ExecutionOptions = {}): Promise<TaskResponse> {
        ...
        try {
            ...
            const result = await this.bridge.post<any>("/api/chat/completions", reqBody);
     ...
     */
    async streamTask(request: TaskRequest, onChunk: (chunk: string) => void, options: ExecutionOptions = {}): Promise<void> {
        if (!this.bridge.isReady()) {
            throw new Error("Python backend is not ready");
        }
